...
export const chains = {
  ...
  [ChainId.ETHEREUM_ROPSTEN]: ChainId.ETHEREUM_ROPSTEN,
...
export const ACCOUNT_PRIVATE_KEYS = requiredEnv("ACCOUNT_PRIVATE_KEY")
  .split(",")
  .map((key) => Buffer.from(key, "hex"));
export const ACCOUNT_WALLETS = new Map(
  ACCOUNT_PRIVATE_KEYS.map((privateKey) => {
...
if (!chains[CHAIN_ID]) {
  throw new Error(
    `Invalid CHAIN_ID: "${CHAIN_ID}". Expected ${Object.keys(chains).join(
      " or "
    )}`
  );
}
